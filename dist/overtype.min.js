...
`):t==="number"&&(h=p.map((c,w)=>/^\s*\d+\.\s+/.test(c)?c.replace(/^(\s*)\d+\.\s+/,"$1"):c.replace(/^(\s*)/,`$1${w+1}. `)).join(`
...
`,r),s=e.slice(a,n),l=s.match(/^(#{1,3})\s/);if(l)if(l[1].length===t){let p=s.replace(/^#{1,3}\s/,"");this.textarea.setRangeText(p,a,n,"end")}else{let p=s.replace(/^#{1,3}/,"#".repeat(t));this.textarea.setRangeText(p,a,n,"end")}else{let p="#".repeat(t)+" "+s;this.textarea.setRangeText(p,a,n,"end")}this.editor.updatePreview()}destroy(){this.shortcuts.clear()}};var d={name:"solar",colors:{bgPrimary:"#faf0ca",bgSecondary:"#ffffff",text:"#0d3b66",h1:"#f95738",h2:"#ee964b",h3:"#3d8a51",strong:"#ee964b",em:"#f95738",link:"#0d3b66",code:"#0d3b66",codeBg:"rgba(244, 211, 94, 0.2)",blockquote:"#5a7a9b",hr:"#5a7a9b",syntaxMarker:"rgba(13, 59, 102, 0.52)",cursor:"#f95738",selection:"rgba(244, 211, 94, 0.4)",listMarker:"#ee964b"}},x={name:"cave",colors:{bgPrimary:"#141E26",bgSecondary:"#1D2D3E",text:"#c5dde8",h1:"#d4a5ff",h2:"#f6ae2d",h3:"#9fcfec",strong:"#f6ae2d",em:"#9fcfec",link:"#9fcfec",code:"#c5dde8",codeBg:"#1a232b",blockquote:"#9fcfec",hr:"#c5dde8",syntaxMarker:"rgba(159, 207, 236, 0.73)",cursor:"#f26419",selection:"rgba(51, 101, 138, 0.4)",listMarker:"#f6ae2d"}},b={solar:d,cave:x,light:d,dark:x};function g(i){return typeof i=="string"?{...b[i]||b.solar,name:i}:i}function k(i){let t=[];for(let[e,r]of Object.entries(i)){let a=e.replace(/([A-Z])/g,"-$1").toLowerCase();t.push(`--${a}: ${r};`)}return t.join(`
`)}function S(i,t={}){return{...i,colors:{...i.colors,...t}}}function L(i={}){let{fontSize:t="14px",lineHeight:e=1.6,fontFamily:r="'SF Mono', Monaco, 'Cascadia Code', 'Roboto Mono', Consolas, 'Courier New', monospace",padding:a="20px",theme:n=null,mobile:s={}}=i,l=Object.keys(s).length>0?`
  ...
  `:"",p=n&&n.colors?k(n.colors):"";return`
